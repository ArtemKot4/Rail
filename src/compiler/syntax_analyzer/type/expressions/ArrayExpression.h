...
struct ArrayExpression : TypeExpression {
    ...
    //is []
    ArrayExpression(std::unique_ptr<Types> t) : type(std::move(t)), limited(false) {}

    //from = 0, to = n is [n-]; from = n1, to = n2 is [n1-n2]
    ArrayExpression(std::unique_ptr<Types> t, int from, int to) : type(std::move(t)), from(from), to(to), limited(true) {}

    //size = n, unlimited = true is [n+]; size = n, unlimied = false is [n]
    ArrayExpression(std::unique_ptr<Types> t, int size, bool limited = true): type(std::move(t)), from(size), to(size), limited(limited) {}

    static std::unique_ptr<Types> parse(SyntaxAnalyzer& analyzer, std::optional<Types> expression) {
        return nullptr;
    }
